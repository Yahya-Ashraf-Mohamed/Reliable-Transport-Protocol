...
def divide_picture_into_packets(picture_path, max_chunk_size):
    with open(picture_path, "rb") as img_file:
        ...
        for i in range(0, len(img_data), max_chunk_bytes):
            ...
            chunk_data_Encrypted = (0).to_bytes(1, byteorder="big")
            'chunk_data_Encrypted = [element ^ Key for element in chunk_data]'
            'print(type(chunk_data))'
            for element in chunk_data:
                'print(type(element))'
                chunk_data_Encrypted += (element ^ Key).to_bytes(1, byteorder="big")
            'print(type(chunk_data_Encrypted))'
            chunks.append(chunk_data_Encrypted)
    return chunks
...
window_size = 20  # sliding window size in go back N protocol
